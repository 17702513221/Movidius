...
from yolov2_tiny_processor import Yolov2_tiny_Processor
import cv2
import numpy
import time
import os
...
NETWORK_GRAPH_FILENAME = "model/tiny_yolo_v2.graph"

# the minimal score for a box to be shown
DEFAULT_INIT_MIN_SCORE = 30
min_score_percent = DEFAULT_INIT_MIN_SCORE

# for title bar of GUI window
cv_window_name = 'video_objects_scalable - yolov2_tiny'
...
def overlay_on_image(display_image:numpy.ndarray, results, fps:float):
    ...
    label_text_color = (255, 0, 0)   # white text	
    if detectedNum > 0:
        for i in range(detectedNum):
            box_xmin = (results[i][0] - results[i][2] / 2.0) * image_width
            box_xmax = (results[i][0] + results[i][2] / 2.0) * image_width
            box_ymin = (results[i][1] - results[i][3] / 2.0) * image_height
            box_ymax = (results[i][1] + results[i][3] / 2.0) * image_height
            ...
            label_text = str(results[i][4]) + " " + str("{0:.2f}".format(results[i][5]*results[i][6]))
            label_background_color = (70, 120, 70) # grayish green background for text
            ...
            #cv2.rectangle(display_image, (label_left - 1, label_top - 5),(label_right + 1, label_bottom + 1),label_background_color, -1)
            # set up text
            cv2.putText(display_image, label_text, (int(box_xmin), int(box_ymin - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.5,label_text_color, 1)
                       
            # set up the rectangle around the object
            cv2.rectangle(display_image, (int(box_xmin), int(box_ymin)), (int(box_xmax), int(box_ymax)), (0, 255, 0), 2)
...
def handle_args():
    ...
    labels = Yolov2_tiny_Processor.get_classification_labels()
